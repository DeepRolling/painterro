...
import {
    genId, addDocumentObjectHelpers, KEYS, trim,
...
class PainterroProc {
    constructor(params) {
        ...
        this.degrees = 0;
        this.setDegreesValue = (newestDegreesValue) => {
            this.degrees = newestDegreesValue;
            console.log('degrees value change : ', this.degrees)
        }
        this.boundaryRad = undefined;
        ...
        this.tools = [{
            ...
            activate: () => {
                ...
                this.closeActiveTool();
                this.setDegreesValue(this.degrees + 90)
            },
        }, {
            name: 'resize',
            activate: () => {
        ...
        this.tools.filter(t => !this.params.hiddenTools.includes(t.name)).forEach((b) => {
            ...
            // console.log(`generate button ${b.name} with id ${id}`);
        ...
        this.worklog = new WorkLog(this, (state) => {
            ...
            (currentState) => {
                if (currentState.operationToolName === 'rotate') {
                    this.setDegreesValue(this.degrees - 90)
                }
            },
            (currentState) => {
                if (currentState.next.operationToolName === 'rotate') {
                    this.setDegreesValue(this.degrees + 90)
                }
            });
        ...
        this.tools.filter(t => this.params.hiddenTools.indexOf(t.name) === -1).forEach((b) => {
            this.getBtnEl(b).onclick = () => {
                ...
                if (currentActive !== b) {
                    console.log('click tool button with target tool: ', b)
                    this.setActiveTool(b);
                } else {
                    console.log('click tool button with default tool: ', this.defaultTool)
    ...
    setToolEnabled(tool, state) {
        if (tool.buttonId) {
            console.log('try to get button id with : ', tool.buttonId)
    ...
    hide() {
        ...
        this.shown = false;
        //SCCT(Source code change tag)
        //don't hide the holder element
